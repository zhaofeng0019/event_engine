...
bool SampleReadFormat::Read(RawStream &raw_stream, uint64_t format) {
    ...
    if ((format & PERF_FORMAT_TOTAL_TIME_ENABLED) &&
        (!raw_stream.Read(&time_enabled_, sizeof(time_enabled_)))) {
        return false;
    }

    if ((format & PERF_FORMAT_TOTAL_TIME_RUNNING) &&
        (!raw_stream.Read(&time_running_, sizeof(time_running_)))) {
        return false;
    }
    ...
    if (format & PERF_FORMAT_GROUP) {
        for (int i = 0; i < nr_or_value; i++) {
            ...
            if ((format & PERF_FORMAT_ID) &&
                !raw_stream.Read(&v.id_, sizeof(v.id_))) {
                return false;
            }
            values_.push_back(v);
        }
...
void Decoder::DecoderFromEvent(std::string group, std::string name,
...
bool PerfSampleRecord::Read(
    RawStream &raw_stream,
    std::unordered_map<uint64_t, perf_event_attr> attr_map,
    perf_event_attr *default_attr,
    std::unordered_map<uint64_t, Decoder> decoder_map) {
    int sample_end;
    ...
    if (default_attr != nullptr) {
        ...
        if ((attr.sample_type & PERF_SAMPLE_IDENTIFIER) &&
            !raw_stream.Read(&sample_id_, sizeof(sample_id_))) {
            goto fail_end;
        }
    } else {
    ...
    if ((attr.sample_type & PERF_SAMPLE_IP) &&
        (!raw_stream.Read(&ip_, sizeof(ip_)))) {
        goto fail_end;
    }

    if ((attr.sample_type & PERF_SAMPLE_TID) &&
        ((!raw_stream.Read(&pid_, sizeof(pid_))) ||
         (!raw_stream.Read(&tid_, sizeof(tid_))))) {
        goto fail_end;
    }

    if ((attr.sample_type & PERF_SAMPLE_TIME) &&
        (!raw_stream.Read(&time_, sizeof(time_)))) {
        goto fail_end;
    }

    if ((attr.sample_type & PERF_SAMPLE_ADDR) &&
        (!raw_stream.Read(&addr_, sizeof(addr_)))) {
        goto fail_end;
    }

    if ((attr.sample_type & PERF_SAMPLE_ID) &&
        (!raw_stream.Read(&id_, sizeof(id_)))) {
        goto fail_end;
    }

    if ((attr.sample_type & PERF_SAMPLE_STREAM_ID) &&
        (!raw_stream.Read(&stream_id_, sizeof(stream_id_)))) {
        goto fail_end;
    }

    if ((attr.sample_type & PERF_SAMPLE_CPU) &&
        (!raw_stream.Read(&cpu_, sizeof(cpu_)))) {
        goto fail_end;
        uint32_t res{0};
    ...
    if ((attr.sample_type & PERF_SAMPLE_PERIOD) &&
        (!raw_stream.Read(&period_, sizeof(period_)))) {
        goto fail_end;
    }

    if ((attr.sample_type & PERF_SAMPLE_READ) &&
        read_format_.Read(raw_stream, attr.read_format)) {
        goto fail_end;
    }
    ...
    if ((attr.sample_type & PERF_SAMPLE_RAW)) {
        ...
        if (raw_stream.GetOffset() - sizeof(raw_data_size_) + raw_data_size_ >
            sample_end) {
            goto fail_end;
        }
        ...
        if (it != decoder_map.end()) {
            ParseRawData(it->second.fields_, raw_data_ptr_, raw_data_size_,
                         raw_data_);
