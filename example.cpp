...
void PrintAcceptEvent(AcceptEvent event)
{
    ...
    char buff[128] = {0};
    std::sprintf(buff, "local_addr %d.%d.%d.%d ", event.local_addr_ & 0xFF, (event.local_addr_ >> 8) & 0xFF, (event.local_addr_ >> 16) & 0xFF, (event.local_addr_ >> 24) & 0xFF);
    std::cout << buff << std::endl;

    std::memset(buff, 0, 128);
    std::sprintf(buff, "remote_addr %d.%d.%d.%d ", event.remote_addr_ & 0xFF, (event.remote_addr_ >> 8) & 0xFF, (event.remote_addr_ >> 16) & 0xFF, (event.remote_addr_ >> 24) & 0xFF);
    std::cout << buff << std::endl;
    ...
    std::memset(buff, 0, 128);
    std::sprintf(buff, "remote_port %d ", ((event.remote_port_ & 0xFF) << 8) + ((event.remote_port_ >> 8) & 0xFF));
    std::cout << buff << std::endl;
...
void ExecHandle(void *data)
{
    ...
    thread_poll.AddTask(std::bind(PrintExecEvent, event));
}
...
int main()
{
    ...
    event_engine::ProbeOption accept_option{
        ...
        .output_ = "ret=$retval remote_addr=+0($retval):u32 local_addr=+4($retval):u32 remote_port=+12($retval):u16 local_port=+14($retval):u16",
        .on_return_ = true,
