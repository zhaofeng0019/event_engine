    ...
    for (auto field : trace_fields) {
        if (field.data_loc_size_ == 0) {
            ...
            if (total_size >= field.offset_ + field.size_) {
                raw_data.field_map_[field.name_] = FieldData{
                    .size_ = field.size_,
        ...
        if (field.data_loc_size_ == 4) /* {u16 offset, u16 len} */
        {
            ...
            uint16_t data_len{0};
            if (!raw_stream.Read(&data_offset, 2) ||
                !raw_stream.Read(&data_len, 2)) {
                continue;
            }
        ...
        uint32_t data_len{0};
        if (!raw_stream.Read(&data_offset, 4) ||
            !raw_stream.Read(&data_len, 4)) {
            continue;
        }
        if (total_size >= data_offset + data_len) {
            raw_data.field_map_[field.name_] = FieldData{
